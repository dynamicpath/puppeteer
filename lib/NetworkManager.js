...
class NetworkManager extends EventEmitter {
  ...
  _onRequestIntercepted(event) {
    ...
    if (event.redirectUrl) {
      ...
      if (request) {
        ...
        this._handleRequestStart(request._requestId, event.interceptionId, event.redirectUrl, event.resourceType, event.request, event.frameId, event.initiator);
      }
      return;
    }
    ...
    if (requestId) {
      ...
      this._handleRequestStart(requestId, event.interceptionId, event.request.url, event.resourceType, event.request, event.frameId, event.initiator);
    } else {
      this._requestHashToInterceptionIds.set(requestHash, event.interceptionId);
      this._handleRequestStart(null, event.interceptionId, event.request.url, event.resourceType, event.request, event.frameId, event.initiator);
    }
  }
   ...
   */
  _handleRequestStart(requestId, interceptionId, url, resourceType, requestPayload, frameId, initiator) {
    let frame = null;
    if (frameId)
  ...
  _onRequestWillBeSent(event) {
    ...
    this._handleRequestStart(event.requestId, null, event.request.url, event.type, event.request, event.frameId, event.initiator);
  }
